...
const WorkoutLog = () => {
    ...
    const Pagination = () => (
        <div className="m-4 flex justify-center items-center">
            <button onClick={() => setCurrentPage(oldPage => Math.max(oldPage - 1, 0))} className="bg-blue-500 text-white py-2 px-4 rounded m-2">Previous</button>
            <span> Page {currentPage + 1} of {totalPages} </span>
            <button onClick={() => setCurrentPage(oldPage => Math.min(oldPage + 1, totalPages - 1))} className="bg-blue-500 text-white py-2 px-4 rounded m-2">Next</button>
        </div>
    );
            ...
            <div className="flex flex-wrap justify-center">
                <div className="mr-5">
                    ...
                    <div className="flex space-x-2 mb-5">
                        ...
                        />
                        <button onClick={addWorkout} className="bg-blue-500 text-white py-2 px-4 rounded">Add Workout</button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 m-4 justify-items-center">
                    {displayedWorkouts.map((workout, index) => (
                        <div key={index} className="mb-4 border p-3 rounded">
                            ...
                            <button onClick={() => deleteWorkout(index)} className="text-red-500">Delete Workout</button>
                        </div>
                    ))}
                </div>
            ) : (
